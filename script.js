...
// @version      1.1.5
// @updateURL
// https://raw.githubusercontent.com/KEN-2000l/EducationPerfected/main/script.js
// @downloadURL
// https://raw.githubusercontent.com/KEN-2000l/EducationPerfected/main/script.js
...
(async function() {
  ...
  function wordList(className) {
    ...
    document.querySelectorAll("div." + className)
        .forEach(i => words.push(i.innerText));
    return words;
  }
  ...
  function findAnswer(question) {
    ...
    if (answer === undefined)
      answer = cutDict[cutString(question)];
    return answer
  }
  ...
  async function correctAnswer(question) {
    ...
    for (let i = 0; i < 300; i++) {
      if (cutString(document.querySelector("td#question-field").innerText) ===
          cutString(question))
        break;
      await sleep(10);
    }
    msg = msg + "Modal question: " +
          document.querySelector("td#question-field").innerText + "\n";
    let answer =
        cutString(document.querySelector("td#correct-answer-field").innerText);
      ...
      try {
        ...
        if (semiTOGGLE === true) {
          try {
            ...
            console.log(
                "if you're on Safari, ignore this error, if not, please report this as a bug at EducationPerfected if something isn't working")
          }
        }
        ...
        if (document.querySelector("td#correct-answer-field") != null &&
            document.querySelector("#question-field") != null) {
  ...
  document.addEventListener("keydown", async (event) => {
    ...
    else if ((event.metaKey && semiTOGGLE === true) ||
             (event.ctrlKey && semiTOGGLE === true)) {
      navigator.clipboard.writeText(answerSub).then(
          function(arr) {},
          function(arr) { console.log(`Failed to Copy Answer: ${arr}`) });
    }
