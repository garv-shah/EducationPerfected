...
(async function () {
  ...
  async function correctAnswer(question) {
    ...
    for (let i = 0; i < 300; i++) {
      if (
        cutString(document.querySelector("td#question-field").innerText) ===
        cutString(question)
      )
        break;
      await sleep(10);
    ...
    msg =
      ...
      "\n";
    let answer = cutString(
      document.querySelector("td#correct-answer-field").innerText
    );
  ...
  async function answerLoop() {
    ...
    while (semiTOGGLE || autoTOGGLE) {
      try {
        ...
        if (
          document.querySelector("td#correct-answer-field") != null &&
