...
function submitButton() {
  if (
    !ifExistsDo("button#explanation-button", (el) => {
      el.click();
    })
  ) {
...
function deleteModals() {
  document
    .querySelectorAll("div[uib-modal-window=modal-window]")
    .forEach((i) => i.remove());
  document
    .querySelectorAll("div[uib-modal-backdrop=modal-backdrop]")
    .forEach((i) => i.remove());
}
...
async function startAnswer() {
  ...
  if (
    document.querySelector(".modal-header.ng-scope") !== null &&
...
const notLoading = () => {
  let spins = Array.from(
    document.querySelectorAll(".EdsSpinner_eds-component_2tfd5")
  );
  let unspins = Array.from(
    document.querySelectorAll('.EdsSpinner_eds-component_2tfd5[block="true"]')
  );
...
async function completeFolder() {
  ...
  const vocabs = [];
  Array.from(document.querySelectorAll('path[fill="#3838ea"]')).forEach((el) =>
    vocabs.push(el.parentNode.parentNode.parentNode.parentNode.parentNode)
  );
  const folders = [];
  Array.from(document.querySelectorAll("div.folder-icon")).forEach((el) =>
    folders.push(el.parentNode.parentNode)
  );
  ...
  for (let j = 0; j < items.length; j++) {
    ...
    await untilElement("div.back-arrow", 3000)
      .then((el) => el.click())
      .catch();
    await sleepUntil(notLoading, 7000).catch();
  }
