...
// @version      2.0.0
// @updateURL
// https://raw.githubusercontent.com/KEN-2000l/EducationPerfected/main/script%2B.js
// @downloadURL
// https://raw.githubusercontent.com/KEN-2000l/EducationPerfected/main/script%2B.js
// @description  Farm Education Perfect points for the Global Leaderboards
...
function deleteModals() {
  document.querySelectorAll('div[uib-modal-window=modal-window]')
      .forEach(i => i.remove());
  document.querySelectorAll('div[uib-modal-backdrop=modal-backdrop]')
      .forEach(i => i.remove());
}
...
async function correctAnswer(question) {
  await sleepUntil(
      () => {return document.querySelector('td#question-field').innerText !==
                    'blau'},
      3000)
      .catch();
  msg = msg + `Extracted Question: ${question}\n`;
  msg = msg + `Correct question: ${
                  document.querySelector('td#question-field').innerText}\n`;
...
async function answerLoop() {
  ...
  while (true) {
    try {
      ...
      await ifExistsDo('td#correct-answer-field',
                       async () => await correctAnswer(question));
...
async function startAnswer() {
  await ifExistsDo('#full-list-switcher', async el => {
    el.click();
    await sleep(200)
  });

  await sleep(400);
  await mergeLists(wordList('div.targetLanguage'),
                   wordList('div.baseLanguage'));
  ...
  if (document.querySelector('.modal-header.ng-scope') !== null &&
      document.querySelector('.modal-header.ng-scope').innerText ===
          'No more stars available') {
    await sleep(200);
...
const notLoading = () => {
  let spins =
      Array.from(document.querySelectorAll('.EdsSpinner_eds-component_2tfd5'));
  let unspins = Array.from(document.querySelectorAll(
      '.EdsSpinner_eds-component_2tfd5[block="true"]'));
...
async function completeFolder() {
  ...
  //     el =>
  //     activities.push(el.parentNode.parentNode.parentNode.parentNode.parentNode));
  const vocabs = [];
  Array.from(document.querySelectorAll('path[fill="#3838ea"]'))
      .forEach(el => vocabs.push(
                   el.parentNode.parentNode.parentNode.parentNode.parentNode));
  const folders = [];
  Array.from(document.querySelectorAll('div.folder-icon'))
      .forEach(el => folders.push(el.parentNode.parentNode));
...
async function completeSubject() {
  untilElement('#subject-browse-button', 5000).then(el => el.click()).catch();
  await untilURLChange(30000).catch();
...
const epHome = () => {
  return Boolean(window.location.href.match(
      /^https?:\/\/app.educationperfect.com\/app\/#\/dashboard\/$/gi))
};
const subjectHome = () => {
  return Boolean(window.location.href.match(
      /^https?:\/\/app.educationperfect.com\/app\/#\/dashboard\/(?!(tasks|competitions|history)\b)\b\w+\/$/gi))
};
const subjectBrowser = () => {
  return Boolean(window.location.href.match(
      /^https?:\/\/app.educationperfect.com\/app\/#\/dashboard\/(?!(tasks|competitions|history)\b)\b\w+\/content\//gi))
};

(async function() {
  document.addEventListener('keydown', async (event) => {
    if ((event.altKey && event.key.toLowerCase() === "s") ||
        (event.key.toLowerCase() === "ß")) {
      // if (epHome()) await completeAll();
      if (subjectHome())
        await completeSubject();
      if (subjectBrowser())
        await completeFolder();
      if (window.location.href.includes('/list-starter'))
        await completeTask();
    }
  });
})();
